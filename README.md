...
| Minecraft 26.1.2 + OptiFine HD U K1 pre1/pre2 | `obj-cubed-optifine-patcher-mc26.1.2-k1-*.jar` | 입력 SHA로 pre1/pre2 자동 감지, 자동 검증 완료 |
| Minecraft 26.2 + OptiFine | 없음 | 공식 OptiFine 26.2가 아직 없어 제작 불가 |
...
```text
java -jar obj-cubed-optifine-patcher-mc26.1.2-k1-0.3.0.jar preview_OptiFine_26.1.2_HD_U_K1_pre2.jar
```

4. 생성된 `*-objcubed.jar`를 평소 OptiFine 설치 파일처럼 직접 실행합니다.

패처는 공식 입력 SHA-256으로 pre1과 pre2를 자동 판별합니다. 원본 JAR은 수정하지 않습니다. 생성된 OptiFine JAR은 개인 사용만 하고 재배포하지 마세요.
...
- OptiFine 패처는 pre1/pre2 SHA 자동 감지와 `srg`/`notch` 양쪽 패치를 검사합니다.
...
- `optifine-patcher/build/libs/obj-cubed-optifine-patcher-mc26.1.2-k1-*.jar`
