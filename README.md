...
Godlander의 obj-cubed 리소스팩을 Iris, Sodium, OptiFine 셰이더 경로에서 디코딩하기 위한 호환 프로젝트입니다. 일반 OpenGL Iris 환경은 Minecraft와 Iris 버전에 맞춘 개별 호환 JAR을 사용하고, Minecraft 26.2 Vulkan만 Iris가 포함된 통합 JAR을 사용합니다.
...
| 26.2 | Fabric + Iris 1.11.2 + Sodium 0.9.1 + OpenGL | `obj-cubed-iris-compat-mc26.2-iris1.11.2-*.jar` | 26.2 전용 ShaderManager 경로 자동 검증 완료, 게임 내 최종 검증 필요 |
| 26.2 | Fabric + Sodium 0.9.1 + Vulkan | `obj-cubed-iris-vulkan-integrated-mc26.2-iris1.11.2-beta.5.jar` | 실제 게임·셰이더·obj-cubed 검증 완료 |
...
일반 Iris JAR은 서로 통합하지 않습니다. 파일명에 적힌 Minecraft와 Iris 버전이 현재 설치 조합과 정확히 일치해야 합니다.
...
26.2 일반 OpenGL도 같은 방식으로 공식 Iris 1.11.2, Sodium 0.9.1과 `obj-cubed-iris-compat-mc26.2-iris1.11.2-*.jar`를 함께 설치합니다.

## Fabric / Iris 26.2 Vulkan 설치

공식 Iris 대신 릴리스의 `obj-cubed-iris-vulkan-integrated-mc26.2-iris1.11.2-beta.5.jar`를 설치합니다. 공식 Iris 및 일반 OpenGL용 `obj-cubed-iris-compat-*.jar`와 동시에 넣으면 안 됩니다. Sodium 0.9.1과 Minecraft 26.2가 필요합니다.
...
```text
java -jar obj-cubed-optifine-patcher-mc26.1.2-k1-pre2-0.2.1.jar preview_OptiFine_26.1.2_HD_U_K1_pre2.jar
```
...
- Fabric 동반 모드는 Minecraft 26.1/26.1.1/26.1.2/26.2와 Iris 1.10.9/1.11.2별로 개별 빌드하며, 각 JAR 내부 의존성도 정확한 Minecraft·Iris 버전으로 제한합니다.
