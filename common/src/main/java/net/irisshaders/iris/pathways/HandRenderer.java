...
public class HandRenderer {
	...
	public void renderSolid(Matrix4fc modelMatrix, float tickDelta, Camera camera, CameraRenderState cameraState, GameRenderer gameRenderer, WorldRenderingPipeline pipeline) {
		renderSolid(modelMatrix, tickDelta, camera, cameraState, gameRenderer, pipeline::setPhase);
	}

	/** Vulkan counterpart of Iris' early opaque-hand pass. */
	public void renderSolidVulkan(Matrix4fc modelMatrix, float tickDelta, Camera camera, CameraRenderState cameraState, GameRenderer gameRenderer) {
		renderSolid(modelMatrix, tickDelta, camera, cameraState, gameRenderer, HandRenderer::setVulkanPhase);
	}

	private void renderSolid(Matrix4fc modelMatrix, float tickDelta, Camera camera, CameraRenderState cameraState,
		GameRenderer gameRenderer, Consumer<WorldRenderingPhase> phase) {
		...
		phase.accept(WorldRenderingPhase.HAND_SOLID);
		...
		phase.accept(WorldRenderingPhase.NONE);
	...
	public void renderTranslucent(Matrix4fc modelMatrix, float tickDelta, Camera camera, CameraRenderState cameraState, GameRenderer gameRenderer, WorldRenderingPipeline pipeline) {
		renderTranslucent(modelMatrix, tickDelta, camera, cameraState, gameRenderer, pipeline::setPhase);
	}

	/** Vulkan counterpart of Iris' late translucent-hand pass. */
	public void renderTranslucentVulkan(Matrix4fc modelMatrix, float tickDelta, Camera camera, CameraRenderState cameraState, GameRenderer gameRenderer) {
		renderTranslucent(modelMatrix, tickDelta, camera, cameraState, gameRenderer, HandRenderer::setVulkanPhase);
	}

	private void renderTranslucent(Matrix4fc modelMatrix, float tickDelta, Camera camera, CameraRenderState cameraState,
		GameRenderer gameRenderer, Consumer<WorldRenderingPhase> phase) {
		...
		phase.accept(WorldRenderingPhase.HAND_TRANSLUCENT);
	...
	private static void setVulkanPhase(WorldRenderingPhase phase) {
		if (phase == WorldRenderingPhase.NONE) {
			IrisVulkan.endHandRendering();
		} else {
			IrisVulkan.beginHandRendering(phase);
		}
	}
