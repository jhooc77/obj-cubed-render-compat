...
public class IrisSamplers {
	...
	public static void addLevelSamplers(SamplerHolder samplers, WorldRenderingPipeline pipeline, AbstractTexture whitePixel, boolean hasTexture, boolean hasLightmap, boolean hasOverlay) {
		// The crumbling render type binds destroy_stage_N as Sampler0, after vanilla
		// has discarded the carrier's atlas UV.  Its preserved UV1 needs a stable,
		// nearest-filtered view of the real block atlas for obj-cubed metadata.
		samplers.addDynamicSampler(
			() -> Minecraft.getInstance().getTextureManager().getTexture(TextureAtlas.LOCATION_BLOCKS)
				.getTexture().iris$getGlId(),
			GlSampler.NEAREST,
			"iris_ObjCubedAtlasSampler"
		);
		if (hasTexture) {
			samplers.addExternalSampler(ALBEDO_TEXTURE_UNIT, "tex", "texture", "gtexture", "u_MainSampler", "iris_ObjCubedSampler");
		} else {
			// TODO: Rebind unbound sampler IDs instead of hardcoding a list...
			samplers.addDynamicSampler(() -> whitePixel.getTexture().iris$getGlId(), GlSampler.NEAREST, "tex", "texture", "gtexture", "u_MainSampler", "iris_ObjCubedSampler",
				"gcolor", "colortex0");
		}
