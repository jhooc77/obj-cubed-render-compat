...
public class CustomUniforms implements FunctionContext {
	...
	/** Updates every value while allowing a backend to isolate unavailable optional inputs. */
	public void updateSafely(BiConsumer<String, Throwable> failureHandler) {
		for (CachedUniform value : this.uniformOrder) {
			try {
				value.update();
			} catch (Throwable throwable) {
				failureHandler.accept(value.getName(), throwable);
			}
		}
	}

	/**
	 * Visits the evaluated fixed inputs and shader-pack variables in dependency order.
	 * Backends which do not use OpenGL uniform locations can use this to serialize the
	 * same values into their native uniform storage.
	 */
	public void forEachValue(Consumer<CachedUniform> consumer) {
		this.uniformOrder.forEach(consumer);
	}
