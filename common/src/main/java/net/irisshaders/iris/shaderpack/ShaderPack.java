...
import net.irisshaders.iris.vulkan.VulkanShaderPackCompatibility;
...
public class ShaderPack {
	...
	public ShaderPack(Path root, Map<String, String> changedConfigs, ImmutableList<StringPair> environmentDefines, boolean isZip) throws IOException, IllegalStateException {
		...
		environmentDefines = VulkanShaderPackCompatibility.apply(root, environmentDefines);
		...
		this.sourceProvider = (path) -> {
			...
			// Shader-pack properties use world0/<program> for the overworld even
			// when the overworld sources live directly in /shaders. In that layout
			// the base ProgramSet asks for "shadowcomp", not
			// "world0/shadowcomp", so honor both spellings. This is particularly
			// important for capability fallbacks which disable an unsupported
			// compute pass while retaining the rest of the pack.
			if (disabledPrograms.contains(programString)
				|| (programString.indexOf('/') < 0 && disabledPrograms.contains("world0/" + programString))) {
				return null;
			}
