...
/**
 * Targeted compatibility bridge for Godlander's obj-cubed resource-pack decoder.
 *
 * <p>Iris normally replaces Minecraft's core entity shader with the active
 * shaderpack program. That means obj-cubed's carrier texture reaches the
 * shaderpack undecoded. This bridge runs the active resource pack's decoder
 * before the shaderpack vertex main and then lets the shaderpack continue with
 * the decoded position, UV and tint.</p>
 */
public final class ObjCubedShaderInjector {
	...
	public static Map<PatchShaderType, String> inject(Map<PatchShaderType, String> transformed, VanillaParameters parameters, String programName) {
		...
		if (tools.isEmpty() || main.isEmpty()) {
			return transformed;
		}

		return injectWithSources(transformed, programName, tools.get(), main.get(),
			isEntityShadowProgram(programName), false);
	}

	/** Injects obj-cubed's BLOCK decoder into a vanilla terrain program. */
	...
	/** Decodes block geometry while leaving UV0 bound to the destroy-stage texture. */
	public static Map<PatchShaderType, String> injectCrumbling(Map<PatchShaderType, String> transformed, String programName) {
		...
		if (!Pattern.compile("(?m)^\\s*(?:layout\\s*\\([^)]*\\)\\s*)?in\\s+[iu]?vec2\\s+iris_UV1\\s*;")
			.matcher(vertex).find()) {
	...
	/** Injects obj-cubed into Minecraft's native destroy-stage shader (shader packs off). */
	public static String injectNativeCrumblingWithSources(String source, String tools, String main) {
		if (source == null || source.contains("iris_ObjCubedDecode") || tools == null || main == null
		...
		String nativeSource = injected
			.replace("uniform sampler2D iris_ObjCubedSampler;",
				"float iris_ObjCubedNativeGameTime() { return GameTime; }\nuniform sampler2D Sampler1;")
	...
	/** Injects obj-cubed after Sodium has unpacked its compact terrain attributes. */
	...
	/** Injects the live decoder into Sodium's own shader when no shader pack is active. */
	public static String injectRawSodiumTerrain(String source) {
		if (source == null || source.contains("iris_ObjCubedDecode")
			|| !Boolean.parseBoolean(System.getProperty("iris.objcubed.compat", "true"))) {
			return source;
		}
		Optional<String> tools = read(TOOLS);
	...
	static Map<PatchShaderType, String> injectWithSources(
		...
		if (injected == null) {
			Iris.logger.warn("obj-cubed {} compatibility was eligible for {} but its transformed attributes/main could not be located",
				blockPass ? "terrain" : "entity", programName);
			return transformed;
		}
	...
	private static boolean isEligible(VanillaParameters parameters, String programName) {
		return Boolean.parseBoolean(System.getProperty("iris.objcubed.compat", "true"))
			// Only entity/equipment carriers use obj-cubed. Do not inject the decoder
			// into terrain, particles, text, or the other shaderpack shadow variants.
	...
	private static String injectVertex(String source, String tools, String decoderBody, boolean shadowPass, boolean blockPass) {
		...
		redirected = originalMain.replaceFirst(Matcher.quoteReplacement(originalMain.group(1)
			+ "void iris_ObjCubedOriginalMain()"));
	...
	private static String injectSodiumTerrainVertex(String source, String tools, String decoderBody) {
		if (!source.contains("_vert_init") || !source.contains("_vert_position")
			|| !source.contains("_vert_tex_diffuse_coord") || !source.contains("u_RegionOffset")) return null;

		Matcher originalMain = MAIN_FUNCTION.matcher(source);
		if (!originalMain.find()) return null;
		String redirected = originalMain.replaceFirst(Matcher.quoteReplacement(originalMain.group(1)
			+ "void iris_ObjCubedOriginalMain()"));
		redirected = Pattern.compile("\\b_vert_init\\s*\\(\\s*\\)\\s*;")
			.matcher(redirected).replaceFirst("");
	...
	private static String redirectInputReads(String source, String type, String inputName, String valueExpression) {
		...
		// Iris binds attributes by their canonical names before program linking.
		// Keep the declaration intact and redirect shaderpack reads through a
		// selector function. Unlike the previous mutable-global bridge, the normal
		// entity branch reads the canonical input directly and cannot inherit stale
		// decoded state on drivers that aggressively scalarize shader globals.
		...
		return redirectedMatcher.replaceFirst(Matcher.quoteReplacement(redirectedMatcher.group(1) + inputName
			+ redirectedMatcher.group(2)));
	}

	private static String renameNativeInput(String source, String from, String to) {
		return Pattern.compile("\\b" + Pattern.quote(from) + "\\b")
			.matcher(source).replaceAll(Matcher.quoteReplacement(to));
	}
	...
	private static String buildSodiumTerrainBridge(String source, String tools, String body) {
		...
		String newMain = """
			void main() {
			    ...
			    // Sodium's packed UV lies on a texel edge. Reapply its per-corner
			    // inward bias before obj-cubed performs integer marker/header fetches.
		...
		String declarations = """
			...
			""";
		if (!Pattern.compile("\\bu_CurrentTime\\b").matcher(source).find()
			&& !Pattern.compile("\\buniform\\s+int\\s+worldTime\\b").matcher(source).find()) {
