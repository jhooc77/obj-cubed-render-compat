...
public class TransformPatcher {
	...
	static {
		...
		transformer.setTransformation((trees, parameters) -> {
			...
			if (IrisLimits.VK_CONFORMANCE || IrisMixinPlugin.usingVulkan) {
	...
	private static Map<PatchShaderType, String> transform(String name, String vertex, String geometry, String tessControl, String tessEval, String fragment,
														  Parameters parameters) {
		return transform(name, vertex, geometry, tessControl, tessEval, fragment, parameters, true);
	}

	private static Map<PatchShaderType, String> transform(String name, String vertex, String geometry, String tessControl, String tessEval, String fragment,
														  Parameters parameters, boolean injectObjCubed) {
		...
		if (injectObjCubed && parameters instanceof VanillaParameters vanillaParameters) {
			// The transform cache can bypass transformInternal(), which is where the
			// mutable Parameters.name field is normally populated. Pass the stable
			// method argument instead so obj-cubed eligibility never dereferences a
			// null cached name.
			return ObjCubedShaderInjector.inject(result, vanillaParameters, name);
		}
		return result;
	}
	...
	public static Map<PatchShaderType, String> patchVanillaWithoutObjCubed(
		String name, String vertex, String geometry, String tessControl, String tessEval, String fragment,
		AlphaTest alpha, boolean isLines, boolean isClouds,
		boolean hasChunkOffset,
		ShaderAttributeInputs inputs,
		Object2ObjectMap<Tri<String, TextureType, TextureStage>, String> textureMap) {
		return transform(name, vertex, geometry, tessControl, tessEval, fragment,
			new VanillaParameters(Patch.VANILLA, textureMap, alpha, isLines, isClouds, hasChunkOffset, inputs,
				geometry != null, tessControl != null || tessEval != null), false);
	}
